...
import {
  MockTarget, MockBorrowSwap, PublicLiquidityPool, MockSignerTrue, MockSignerFalse, TransparentUpgradeableProxy,
...
describe("PublicLiquidityPool", function () {
  ...
  const deployAll = async () => {
    ...
    const liquidityPoolInit = (await liquidityPoolImpl.initialize.populateTransaction(
      admin, mpc_signer, mockSignerTrue, "Public Liquidity Pool", "PLP", 2000
    )).data;
    ...
    return {deployer, admin, user, user2, mpc_signer, usdc, usdcOwner, gho, ghoOwner, eurc, eurcOwner,
      ...
      weth, wethOwner, feeSetter, withdrawProfit, pauser, mockSignerTrue, mockSignerFalse, lp, directBorrower};
  };

  describe("Initialization", function () {
    it("Should initialize the contract with correct values", async function () {
      const {
        liquidityPool, liquidityPoolImpl, usdc, admin, mpc_signer, mockSignerTrue, USDC_DEC, user
    ...
    it("Should NOT deploy the proxy if admin address is 0", async function () {
      ...
      await expect(deploy(
        ...
        (await liquidityPool.initialize.populateTransaction(
          ZERO_ADDRESS, mpc_signer, mockSignerTrue, "Public Liquidity Pool", "PLP", 2000
        )).data
      )).to.be.revertedWithCustomError(liquidityPool, "ZeroAddress");
    });
    ...
    it("Should NOT deploy the proxy if MPC address is 0", async function () {
      ...
      await expect(deploy(
        ...
        liquidityPoolImpl, admin,
        (await liquidityPool.initialize.populateTransaction(
          admin, ZERO_ADDRESS, mockSignerTrue, "Public Liquidity Pool", "PLP", 2000
        )).data
      )).to.be.revertedWithCustomError(liquidityPool, "ZeroAddress");
    });
    ...
    it("Should NOT deploy the proxy if signer address is 0", async function () {
      ...
      await expect(deploy(
        ...
        liquidityPoolImpl, admin,
        (await liquidityPool.initialize.populateTransaction(
          admin, mpc_signer, ZERO_ADDRESS, "Public Liquidity Pool", "PLP", 2000
        )).data
      )).to.be.revertedWithCustomError(liquidityPool, "ZeroAddress");
    });

    it("Should NOT deploy the proxy if protocol fee rate is greater than 10000", async function () {
      const {deployer, liquidityPool, liquidityPoolImpl, admin, mpc_signer,
        mockSignerTrue} = await loadFixture(deployAll);
      await expect(deploy(
        "TransparentUpgradeableProxy", deployer, {},
        liquidityPoolImpl, admin,
        (await liquidityPool.initialize.populateTransaction(
          admin, mpc_signer, mockSignerTrue, "Public Liquidity Pool", "PLP", 10001
        )).data
  ...
  describe("Borrow, supply, withdraw", function () {
    ...
    it("Should withdraw profit for multiple tokens from the pool", async function () {
      ...
      await expect(tx).to.emit(liquidityPool, "ProfitWithdrawn").withArgs(eurc, user, amountEurc);
      await expect(tx).to.emit(liquidityPool, "ProfitWithdrawn").withArgs(gho, user, amountGho);
