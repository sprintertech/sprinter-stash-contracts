...
abstract contract GnosisOmnibridgeAdapter is AdapterHelper {
    ...
    function processTransferGnosisOmnibridge(
        ...
        if (localDomain == Domain.GNOSIS_CHAIN) {
            token = IERC20(abi.decode(extraData, (address)));
            require(address(GNOSIS_USDCXDAI) != address(0), ZeroAddress());
            require(GNOSIS_USDCXDAI == token, InvalidOutputToken());
            amount = GNOSIS_USDCXDAI.balanceOf(address(this));
            require(amount > 0, ZeroBalance());
            IUSDCTransmuter usdceSwap = GNOSIS_USDC_TRANSMUTER;
            GNOSIS_USDCXDAI.forceApprove(address(usdceSwap), amount);
            usdceSwap.deposit(amount);
            token = GNOSIS_USDCE;
