...
async function fetchTxStateChanges(txHash: string, networkId: string): Promise<TxResult> {
  ...
  const url = `https://api.tenderly.co/api/v1/account/${TENDERLY_ACCOUNT}/project/${TENDERLY_PROJECT}/network/${networkId}/trace/${txHash}`;
...
async function main() {
  ...
  for (const tx of transactions) {
    ...
    results.push(result);
    console.log(`  ${result.error ? `Error: ${result.error}` : `${result.status ? "Success" : "Failed"}, ${result.stateChanges.length} addresses`}`);
  }
