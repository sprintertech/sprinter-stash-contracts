...
import {
  deploy, getContractAt, signBorrow, getBalance, signBorrowMany,
  setupTests, BLOCK_TAG, packAmount, expectAlmostEqualDown,
} from "./helpers";
...
describe("LiquidityPoolAaveLongTerm", function () {
  ...
  const deployAll = async () => {
    ...
    const wethData = await aavePool.getReserveData(WETH_ADDRESS);
    const wethDebtToken = await hre.ethers.getContractAt("ERC20", wethData[10]);
    ...
    const liquidityPool = (
      await getContractAt("LiquidityPoolAaveLongTerm", liquidityPoolProxy, deployer)
    ) as LiquidityPoolAaveLongTerm;
    ...
    const REPAYER_ROLE = encodeBytes32String("REPAYER_ROLE");
    ...
    return {
      ...
      weth, wethOwner, WETH_DEC, wethDebtToken, REPAYER_ROLE, BORROW_LONG_TERM_ROLE, borrowAdmin,
      directBorrower, mockSignerTrue, mockSignerFalse
    };
  };
  ...
  describe("Borrow, supply, repay, withdraw", function () {
    ...
    it("Should borrow a non-supported token", async function () {
      const {
        liquidityPool, usdc, usdcOwner, USDC_DEC, nonSupportedToken, nonSupportedTokenOwner,
        liquidityAdmin, mockTarget, user, mpc_signer,
      } = await loadFixture(deployAll);
      ...
      const amount = 3n * USDC_DEC;
      await nonSupportedToken.connect(nonSupportedTokenOwner).transfer(liquidityPool, amount);
      const amountToBorrow = 1n * USDC_DEC;

      const callData = await mockTarget.fulfill.populateTransaction(nonSupportedToken, amountToBorrow, "0x");
      ...
      await liquidityPool.connect(user).borrow(
        ...
        2000000000n,
        signature);
      expect(await nonSupportedToken.balanceOf(liquidityPool)).to.eq(amount - amountToBorrow);
      expect(await nonSupportedToken.balanceOf(mockTarget)).to.eq(amountToBorrow);

      expectAlmostEqual(await liquidityPool.balance(usdc), 50n * USDC_DEC);
      expect(await liquidityPool.balance(nonSupportedToken)).to.eq(amount - amountToBorrow);
    });
    ...
    it("Should borrow a token without Aave if have enough balance", async function () {
      const {
        liquidityPool, usdc, USDC_DEC, user, user2, mpc_signer, usdcOwner,
        liquidityAdmin, usdcDebtToken
      } = await loadFixture(deployAll);
      const amountCollateral = 1000n * USDC_DEC;
      await usdc.connect(usdcOwner).transfer(liquidityPool, amountCollateral);
      await expect(liquidityPool.connect(liquidityAdmin).deposit(amountCollateral))
      ...
      expect(await usdcDebtToken.balanceOf(liquidityPool)).to.eq(0);
    });
    ...
    it("Should borrow a native token without Aave if have enough balance with contract call", async function () {
      const {
        liquidityPool, mockTarget, usdc, USDC_DEC, weth, user, mpc_signer, usdcOwner, liquidityAdmin, wethDebtToken
      } = await loadFixture(deployAll);
      const amountLiquidity = 100000n * USDC_DEC;
      await usdc.connect(usdcOwner).transfer(liquidityPool, amountLiquidity);
      await expect(liquidityPool.connect(liquidityAdmin).deposit(amountLiquidity));
      ...
      expect(await getBalance(mockTarget)).to.eq(amountToBorrow);
      expect(await liquidityPool.balance(NATIVE_TOKEN)).to.eq(await liquidityPool.balance(weth));
      expect(await wethDebtToken.balanceOf(liquidityPool)).to.eq(0);
    });
    ...
    it("Should borrow many tokens without Aave if have enough balance", async function () {
      const {
        liquidityPool, usdc, USDC_DEC, gho, GHO_DEC, user, user2, mpc_signer, usdcOwner,
        liquidityAdmin, ghoOwner, ghoDebtToken, usdcDebtToken
      } = await loadFixture(deployAll);
      const amountCollateral = 1000n * USDC_DEC;
      await usdc.connect(usdcOwner).transfer(liquidityPool, amountCollateral);
      await liquidityPool.connect(liquidityAdmin).deposit(amountCollateral);
      ...
      await usdc.connect(usdcOwner).transfer(liquidityPool, amountToBorrow2);
      ...
      expect(await gho.balanceOf(liquidityPool)).to.eq(amountToBorrow);
      expect(await usdc.balanceOf(liquidityPool)).to.eq(amountToBorrow2);
      expect(await ghoDebtToken.balanceOf(liquidityPool)).to.eq(0);
      expect(await usdcDebtToken.balanceOf(liquidityPool)).to.eq(0);
    });
    ...
    it("Should borrow a token without Aave if have enough balance with swap", async function () {
      // GHO is borrowed and swapped to EURC
      const {
        liquidityPool, mockTarget, mockBorrowSwap, usdc, USDC_DEC, gho, GHO_DEC,
        user, mpc_signer, usdcOwner, eurc, EURC_DEC, eurcOwner, liquidityAdmin, ghoOwner, ghoDebtToken
      } = await loadFixture(deployAll);
      const amountCollateral = 1000n * USDC_DEC; // $1000
      await usdc.connect(usdcOwner).transfer(liquidityPool, amountCollateral);
      await expect(liquidityPool.connect(liquidityAdmin).deposit(amountCollateral))
    ...
    it("Should NOT borrow many tokens with duplicates [weth, native]", async function () {
      const {
      ...
      // A total of 4 WETH is borrowed and sent to the target.
      expect(await liquidityPool.totalDeposited()).to.eq(amountLiquidity);
      expect(await weth.balanceOf(liquidityPool)).to.eq(0);
    ...
    it("Should NOT borrow many tokens with duplicates [native, weth]", async function () {
      const {
        liquidityPool, mockTarget, weth, WETH_DEC, user, mpc_signer, liquidityAdmin,
        usdc, usdcOwner, USDC_DEC,
      } = await loadFixture(deployAll);
      ...
      // A total of 4 WETH is borrowed and 3 ETH is sent to the target.
      expect(await liquidityPool.totalDeposited()).to.eq(amountLiquidity);
      expect(await weth.balanceOf(liquidityPool)).to.eq(1n * WETH_DEC);
      expect(await weth.balanceOf(mockTarget)).to.eq(0);
      expect(await getBalance(liquidityPool)).to.eq(0);
      expect(await getBalance(mockTarget)).to.eq(amountToBorrow);
    });

    it("Should NOT borrow many tokens with duplicates [usdc, weth, usdc]", async function () {
      const {
        liquidityPool, mockTarget, weth, WETH_DEC, user, mpc_signer, liquidityAdmin,
        usdc, usdcOwner, USDC_DEC,
      } = await loadFixture(deployAll);
      ...
      const amountToBorrow = 10n * USDC_DEC;
      const amountToBorrow2 = 4n * WETH_DEC;
      const amountToBorrow3 = 20n * USDC_DEC;
      ...
      // A total of 20 USDC is borrowed.
      expect(await liquidityPool.totalDeposited()).to.eq(amountLiquidity);
      expect(await weth.balanceOf(liquidityPool)).to.eq(amountToBorrow2);
      expect(await weth.balanceOf(mockTarget)).to.eq(0);
      expect(await usdc.balanceOf(liquidityPool)).to.eq(amountToBorrow3);
      expect(await usdc.balanceOf(mockTarget)).to.eq(0);
      expect(await getBalance(liquidityPool)).to.eq(0);
      expect(await getBalance(mockTarget)).to.eq(0);
    });

    it("Should borrow many tokens [weth, usdc] with contract call", async function () {
      const {
        liquidityPool, mockTarget, weth, WETH_DEC, user, mpc_signer, liquidityAdmin,
        usdc, usdcOwner, USDC_DEC,
      } = await loadFixture(deployAll);
    ...
    it("Should borrow many tokens [native, usdc] with contract call", async function () {
      const {
        liquidityPool, mockTarget, weth, user, mpc_signer, liquidityAdmin,
        usdc, usdcOwner, USDC_DEC,
      } = await loadFixture(deployAll);
      const amountLiquidity = 1000000n * USDC_DEC;
      await usdc.connect(usdcOwner).transfer(liquidityPool, amountLiquidity);
    ...
    it("Should borrow many tokens [usdc, weth, gho] with contract call", async function () {
      const {
        liquidityPool, mockTarget, weth, WETH_DEC, user, mpc_signer, liquidityAdmin,
        usdc, usdcOwner, USDC_DEC, gho, GHO_DEC
      } = await loadFixture(deployAll);
      const amountLiquidity = 1000000n * USDC_DEC;
      await usdc.connect(usdcOwner).transfer(liquidityPool, amountLiquidity);
      await expect(liquidityPool.connect(liquidityAdmin).deposit(amountLiquidity));
      ...
      const amountToBorrow2 = 4n * WETH_DEC;
      const amountToBorrow3 = 2n * GHO_DEC;
      ...
      expect(await getBalance(mockTarget)).to.eq(0);
      expect(await gho.balanceOf(liquidityPool)).to.eq(0);
      expect(await gho.balanceOf(mockTarget)).to.eq(amountToBorrow3);
    });
    ...
    it("Should borrow many tokens without Aave if have enough balance with swap", async function () {
      // GHO is borrowed and swapped to EURC
      const {
        liquidityPool, mockTarget, mockBorrowSwap, usdc, USDC_DEC, gho, GHO_DEC,
        user, mpc_signer, usdcOwner, eurc, EURC_DEC, eurcOwner, liquidityAdmin, ghoOwner,
        ghoDebtToken, usdcDebtToken
      } = await loadFixture(deployAll);
      const amountCollateral = 1000n * USDC_DEC; // $1000
      await usdc.connect(usdcOwner).transfer(liquidityPool, amountCollateral);
      await liquidityPool.connect(liquidityAdmin).deposit(amountCollateral);
    ...
    it("Should NOT repay a debt without repayer role", async function () {
      const {liquidityPool, gho, user2} = await loadFixture(deployAll);

      await expect(liquidityPool.connect(user2).repay([gho]))
      .to.be.revertedWithCustomError(liquidityPool, "AccessControlUnauthorizedAccount");
    });

    it("Should NOT repay a debt partially without repayer role", async function () {
      const {liquidityPool, gho, user2} = await loadFixture(deployAll);

      await expect(liquidityPool.connect(user2).repayPartial([gho], [1n]))
      .to.be.revertedWithCustomError(liquidityPool, "AccessControlUnauthorizedAccount");
    });
    ...
    it("Should repay a debt", async function () {
      ...
      expect(await liquidityPool.balance(eurc)).to.be.lessThan(availableBefore + amountToBorrow + 1n * EURC_DEC);
      expect(await liquidityPool.balance(eurc)).to.be.greaterThan(availableBefore + amountToBorrow - 1n * EURC_DEC);
    });

    it("Should repay a debt partially", async function () {
      const {
        liquidityPool, usdc, eurc, mpc_signer, user, user2, usdcOwner, eurcOwner, liquidityAdmin, USDC_DEC, EURC_DEC,
        aavePool,
      } = await loadFixture(deployAll);
      ...
      await time.increase(3600);
      await expect(liquidityPool.connect(user).repayPartial([eurc], [amountToBorrow]))
        .to.emit(liquidityPool, "Repaid");
      expect(await eurc.allowance(liquidityPool, aavePool)).to.eq(0);
      expect(await eurc.balanceOf(liquidityPool)).to.eq(amountToBorrow);
      expect(await liquidityPool.balance(eurc)).to.be.lessThan(availableBefore + amountToBorrow + 1n * EURC_DEC);
    ...
    it("Should repay a debt partially when the contract is paused", async function () {
      const {
        liquidityPool, usdc, eurc, mpc_signer, user, user2, usdcOwner, eurcOwner, liquidityAdmin, USDC_DEC, EURC_DEC,
        aavePool, pauser,
      } = await loadFixture(deployAll);
    ...
    it("Should repay partially if some tokens don't have debt", async function () {
      const {
        liquidityPool, usdc, USDC_DEC, gho, GHO_DEC, eurc, user, user2, mpc_signer, usdcOwner, eurcOwner,
        liquidityAdmin, ghoOwner, ghoDebtToken, eurcDebtToken, EURC_DEC
      } = await loadFixture(deployAll);
      ...
      await expect(liquidityPool.connect(user).repayPartial([eurc, gho], [1n * EURC_DEC / 2n, amountToBorrow / 2n]))
        .to.emit(liquidityPool, "Repaid");
      const eurcDebtAfter = await eurcDebtToken.balanceOf(liquidityPool);
      expect(eurcDebtAfter).to.eq(0);
      const ghoDebtAfter = await ghoDebtToken.balanceOf(liquidityPool);
      expect(ghoDebtAfter).to.be.greaterThan(amountToBorrow / 2n);
      expect(ghoDebtAfter).to.be.lessThan(amountToBorrow);
    });
    ...
    it("Should repay collateral partially", async function () {
      const {
        liquidityPool, usdc, USDC_DEC, user, user2, mpc_signer, usdcOwner, liquidityAdmin,
        usdcDebtToken
      } = await loadFixture(deployAll);
      ...
      await expect(liquidityPool.connect(user).repayPartial([usdc], [amountToBorrow / 2n]))
        .to.emit(liquidityPool, "Repaid");
      const usdcDebtAfter = await usdcDebtToken.balanceOf(liquidityPool);
      expect(usdcDebtAfter).to.be.greaterThan(amountToBorrow / 2n);
      expect(usdcDebtAfter).to.be.lessThan(amountToBorrow);
      expect(await liquidityPool.balance(usdc)).to.be.greaterThanOrEqual(50n * USDC_DEC - 1n);
    });
    ...
    it("Should return present balance for a non-supported token", async function () {
      const {
        liquidityPool, usdc, usdcOwner, USDC_DEC, nonSupportedToken, nonSupportedTokenOwner,
        liquidityAdmin
      } = await loadFixture(deployAll);
      ...
      await nonSupportedToken.connect(nonSupportedTokenOwner).transfer(liquidityPool, amount);
      expectAlmostEqual(await liquidityPool.balance(usdc), 50n * USDC_DEC);
      expect(await liquidityPool.balance(nonSupportedToken)).to.eq(amount);
    });
    ...
    it("Should borrow without Aave if health factor is too low", async function () {
      const {
        liquidityPool, admin, usdc, eurc, mpc_signer, user, user2, usdcOwner,
        USDC_DEC, EURC_DEC, liquidityAdmin,
      } = await loadFixture(deployAll);
      const amountCollateral = 1000n * USDC_DEC;
      await usdc.connect(usdcOwner).transfer(liquidityPool, amountCollateral);
      ...
      // Verify that it reverts if the borrow from Aave happen for the first token.
      await expect(liquidityPool.connect(user).borrow(
        eurc,
        amountToBorrow + 1n,
        user2,
    ...
    it("Should borrow many without Aave if health factor is too low", async function () {
      const {
        liquidityPool, admin, usdc, eurc, gho, mpc_signer, user, user2, usdcOwner,
        USDC_DEC, EURC_DEC, GHO_DEC, liquidityAdmin,
      } = await loadFixture(deployAll);
      ...
      // Verify that it reverts if the borrow from Aave happen for the second token.
      await expect(liquidityPool.connect(user).borrowMany(
        [eurc, gho],
        [amountToBorrow, amountToBorrow2 + 1n],
        user2,
    ...
    it("Should skip ltv check if borrowed without Aave", async function () {
      const {
        liquidityPool, usdc, USDC_DEC, user, user2, mpc_signer, usdcOwner,
        liquidityAdmin, usdcDebtToken, admin
      } = await loadFixture(deployAll);
      const amountCollateral = 1000n * USDC_DEC;
      await usdc.connect(usdcOwner).transfer(liquidityPool, amountCollateral);
      await expect(liquidityPool.connect(liquidityAdmin).deposit(amountCollateral))
      ...
      await liquidityPool.connect(user).borrow(
        ...
        signature);
      expect(await usdc.balanceOf(liquidityPool)).to.eq(amountToBorrow);
      expect(await usdcDebtToken.balanceOf(liquidityPool)).to.be.greaterThanOrEqual(amountToBorrow);
      await liquidityPool.connect(admin).setBorrowTokenLTVs([usdc], [1n]);
      signature = await signBorrow(
        mpc_signer,
        liquidityPool,
        user,
      ...
      // Verify that it reverts if the borrow from Aave happen.
      await expect(liquidityPool.connect(user).borrow(
        usdc,
        amountToBorrow + 1n,
        user2,
    ...
    it("Should skip ltv check if set to 100%", async function () {
      const {
        liquidityPool, admin, usdc, eurc, mpc_signer, user, usdcOwner, USDC_DEC, EURC_DEC, liquidityAdmin,
        mockTarget
      } = await loadFixture(deployAll);
      const amountCollateral = 1000n * USDC_DEC; // $1000
      await usdc.connect(usdcOwner).transfer(liquidityPool, amountCollateral);
      ...
      const callData = await mockTarget.fulfill.populateTransaction(eurc, amountToBorrow, "0x");
      ...
      await time.increase(3600);
      // Extra warmup so that profit field and debt accounting are warmed up.
      await liquidityPool.connect(user).borrow(
        eurc,
        amountToBorrow,
        mockTarget,
    ...
    it("Should NOT repay partially if all tokens don't have debt or balance", async function () {
      const {
        liquidityPool, usdc, USDC_DEC, gho, GHO_DEC, eurc, user, mockTarget,
        mpc_signer, usdcOwner, eurcOwner, liquidityAdmin, EURC_DEC
      } = await loadFixture(deployAll);
      ...
      // No balance for gho, no dept for eurc
      await expect(liquidityPool.connect(user).repayPartial([eurc, gho], [1n, 1n]))
        .to.be.revertedWithCustomError(liquidityPool, "NothingToRepay");
    });
    ...
    it("Should NOT repay unsupported tokens partially", async function () {
      const {liquidityPool, user} = await loadFixture(deployAll);
      const unsupportedToken = await hre.ethers.getContractAt("ERC20", "0x53fFFB19BAcD44b82e204d036D579E86097E5D09");

      // No balance for gho, no dept for eurc
      await expect(liquidityPool.connect(user).repayPartial([unsupportedToken], [1n]))
        .to.be.revertedWithCustomError(liquidityPool, "NothingToRepay");
    });
    ...
    it("Should withdraw profit if the token balance is greater than debt", async function () {
      ...
      await expect(liquidityPool.connect(withdrawProfit).withdrawProfit([gho], user))
        .to.be.revertedWithCustomError(liquidityPool, "NoProfit()");
      const profitAmount = 10n * GHO_DEC;
      await gho.connect(ghoOwner).transfer(liquidityPool, profitAmount);
      await expect(liquidityPool.connect(withdrawProfit).withdrawProfit([eurc, gho], user))
        .to.emit(liquidityPool, "ProfitWithdrawn");
      expect(await gho.balanceOf(user)).to.be.greaterThan(profitAmount - 1n * GHO_DEC);
      expect(await gho.balanceOf(user)).to.be.lessThanOrEqual(profitAmount);
      expect(await eurc.balanceOf(user)).to.eq(0);
    });
    ...
    it("Should record negative profit if interest < accrued debt", async function () {
      ...
      await time.setNextBlockTimestamp(await time.latest() + (3600 * 24 * 60));
    ...
    it("Should withdraw other token profit if direct debt is greater than aave debt", async function () {
      ...
      // virtualBalance ≈ 3 > 0+, profit ≈ 3 GHO (pool balance after repay)
    ...
    it("Should withdraw asset token profit if direct debt is greater than aave debt", async function () {
      ...
      // totalBalance ≈ 1 USDC + aToken interest, virtualBalance ≈ 3 > 0+, profit ≈ 3 USDC

      await expect(liquidityPool.connect(withdrawProfit).withdrawProfit([usdc], user))
        .to.emit(liquidityPool, "ProfitWithdrawn");
      // USDC aToken interest may add a small amount to profit
      expectAlmostEqual(await usdc.balanceOf(user), 3n * USDC_DEC, 100n);
    });
  ...
  describe("withdrawProfit - ASSET token (USDC) accrued profit", function () {
    ...
    it("5: USDC borrow with profit, debt stays outstanding, 1hr → profit+interest via shortfall", async function () {
      const fixture = await loadFixture(deployAll);
      const {
        liquidityPool, usdc, usdcOwner, USDC_DEC, aToken, liquidityAdmin, withdrawProfit, user,
      } = fixture;
      ...
      // Borrower returns funds; pool holds them without forwarding to Aave (debt stays live).
      await usdc.connect(usdcOwner).transfer(liquidityPool, borrowAmount + profit);

      await time.setNextBlockTimestamp(await time.latest() + 3600);
      const interest = await aToken.balanceOf(liquidityPool, BLOCK_TAG) - deposit;
      // profit_net = accruedProfit + interest - fees; balance = borrowAmount+profit+interest → no shortfall
    ...
    it("6: USDC Aave debt fees > aToken yield → profit negative, accruedProfit saved", async function () {
      ...
      const fees = await usdcDebtToken.balanceOf(liquidityPool, BLOCK_TAG)
        - regularBorrowAmount - directBorrowAmount;
      expect(fees).to.be.greaterThan(interest);

      await liquidityPool.connect(withdrawProfit).withdrawProfit([usdc, eurc], user);
      expect(await liquidityPool.accruedProfit(usdc)).to.be.lessThan(0n);
      expect(await usdc.balanceOf(user)).to.eq(0n);
      expect(await eurc.balanceOf(user)).to.eq(eurcDonation);
      expect(await usdc.balanceOf(liquidityPool)).to.eq(interest);
    });

    it("7: negative accruedProfit deepens each period as fees accumulate without repayment", async function () {
      const fixture = await loadFixture(deployAll);
      const {
    ...
    it("8: borrowDirect + donated USDC → directDebt surplus floor exceeds explicit profit", async function () {
      ...
      expectAlmostEqualDown(await liquidityPool.accruedProfit(usdc), profit, 2n);
  ...
  describe("withdrawProfit - non-ASSET token (EURC) accrued profit", function () {
    ...
    it("2: EURC borrow with profit, full repayment transferred but Aave debt kept live", async function () {
      const fixture = await loadFixture(deployAll);
      const {
      ...
      // Borrower returns full repayment; pool holds it without forwarding to Aave.
      // Aave EURC debt (borrowAmount + fees) stays outstanding.
      await eurc.connect(eurcOwner).transfer(liquidityPool, borrowAmount + profit);
      // pool EURC = borrowAmount+profit; surplus = profit - fees ≈ profit → no shortfall.
      // profit_net = accruedProfit - fees ≈ accruedProfit; balance ≥ profit_net.
    ...
    it("3: EURC borrow with profit, nothing returned → profit drawn via Aave shortfall borrow", async function () {
      const fixture = await loadFixture(deployAll);
      const {
    ...
    it("4: large EURC borrow, tiny profit, Aave fees > profit → negative accruedProfit saved", async function () {
      ...
      expect(eurcFees).to.be.greaterThan(profit);
      ...
      expect(await usdc.balanceOf(user)).to.eq(usdcInterest);
    });

    it("5: recovery via new profitable EURC borrow after negative accruedProfit, without repayment", async function () {
      const fixture = await loadFixture(deployAll);
      const {
        liquidityPool, usdc, eurc, usdcOwner, eurcOwner, USDC_DEC, EURC_DEC,
        aToken, liquidityAdmin, withdrawProfit, user,
      } = fixture;
      ...
      // First period: commit negative accruedProfit[eurc] via USDC co-token.
      // Borrower returns full repayment; pool holds it, Aave debt stays live.
      await borrowEURCFromAave(fixture, 10n * EURC_DEC, 10n);
      await eurc.connect(eurcOwner).transfer(liquidityPool, 10n * EURC_DEC + 10n);
      await time.setNextBlockTimestamp(await time.latest() + 3600);
      const usdcInterest = await aToken.balanceOf(liquidityPool, BLOCK_TAG) - deposit;
      await liquidityPool.connect(withdrawProfit).withdrawProfit([eurc, usdc], user);
      expect(await liquidityPool.accruedProfit(eurc)).to.be.lessThan(0n);

      // Without repaying the outstanding EURC debt, make a new profitable EURC borrow.
      // The new debtSnapshot absorbs accumulated fees; the large profit turns net positive.
      const recoverProfit = EURC_DEC / 10n; // 0.1 EURC >> accumulated deficit (~50 units)
      await borrowEURCFromAave(fixture, 1n * EURC_DEC, recoverProfit, 1n);
      // accruedProfit[eurc] = previous_negative + recoverProfit ≈ recoverProfit (positive)
