...
import {
  deploy, getContractAt, signBorrow, signBorrowMany, getBalance,
  setupTests, packAmount,
} from "./helpers";
...
describe("LiquidityPool", function () {
  ...
  const deployAll = async () => {
    ...
    return {deployer, admin, user, user2, mpc_signer, usdc, usdcOwner, gho, ghoOwner, eurc, eurcOwner,
      ...
      weth, wethOwner, liquidityAdmin, withdrawProfit, pauser, directBorrower, mockSignerTrue, mockSignerFalse};
  };
  ...
  describe("Initialization", function () {
    ...
    it("Should deploy the contract if wrapped native token address is 0", async function () {
    ...
    it("Should NOT deploy the proxy if signer address is 0", async function () {
      const {deployer, liquidityPool, liquidityPoolImpl, admin, mpc_signer} = await loadFixture(deployAll);
      await expect(deploy(
        "TransparentUpgradeableProxy", deployer, {},
        liquidityPoolImpl, admin,
        (await liquidityPool.initialize.populateTransaction(admin, mpc_signer, ZERO_ADDRESS)).data
      )).to.be.revertedWithCustomError(liquidityPool, "ZeroAddress");
    });
  });
  ...
  describe("Borrow, supply, withdraw", function () {
    ...
    it("Should revert during withdrawing profit if balance is less than total deposited", async function () {
      const {
        liquidityPool, usdc, USDC_DEC, usdcOwner, user, liquidityAdmin, withdrawProfit, mockTarget, mpc_signer,
      } = await loadFixture(deployAll);

      const amountLiquidity = 1000n * USDC_DEC;
      await usdc.connect(usdcOwner).transfer(liquidityPool, amountLiquidity);
      await liquidityPool.connect(liquidityAdmin).deposit(amountLiquidity);
      // totalDeposited = 1000, balance = 1000
      ...
      // balance = 900, directDebt = 0, virtualBalance = 900 < 1000 = totalDeposited

      await expect(liquidityPool.connect(withdrawProfit).withdrawProfit([usdc], user))
        .to.be.revertedWithCustomError(liquidityPool, "NoProfit()");
    });

    it("Should revert withdrawing profit if balance plus direct debt < total deposited", async function () {
      const {
        liquidityPool, usdc, USDC_DEC, usdcOwner, user, liquidityAdmin, withdrawProfit,
        directBorrower, mockTarget, mpc_signer,
      } = await loadFixture(deployAll);
      ...
      // balance = 700, directDebt = 300, virtualBalance = 1000
      ...
      // balance = 600, directDebt = 300, virtualBalance = 900 < 1000 = totalDeposited

      await expect(liquidityPool.connect(withdrawProfit).withdrawProfit([usdc], user))
        .to.be.revertedWithCustomError(liquidityPool, "NoProfit()");
    });

    it("Should withdraw profit if balance plus direct debt is greater than total deposited", async function () {
      const {
        liquidityPool, usdc, USDC_DEC, usdcOwner, user, liquidityAdmin, withdrawProfit, directBorrower,
      } = await loadFixture(deployAll);
      ...
      const extraProfit = 100n * USDC_DEC;
      await usdc.connect(usdcOwner).transfer(liquidityPool, extraProfit);
      // balance = 800, virtualBalance = 1100 > 1000, profit = Math.min(800, 100) = 100

      await expect(liquidityPool.connect(withdrawProfit).withdrawProfit([usdc], user))
        .to.emit(liquidityPool, "ProfitWithdrawn").withArgs(usdc.target, user.address, extraProfit);
      expect(await usdc.balanceOf(user)).to.eq(extraProfit);
    });

    it("Should withdraw profit if direct debt is greater than total deposited", async function () {
      const {
        liquidityPool, usdc, USDC_DEC, usdcOwner, user, liquidityAdmin, withdrawProfit, directBorrower,
      } = await loadFixture(deployAll);

      // Transfer extra USDC (untracked) and deposit a smaller amount
      const depositAmount = 100n * USDC_DEC;
      const extraAmount = 400n * USDC_DEC;
      ...
      // balance = 500, totalDeposited = 100

      // directDebt (200) > totalDeposited (100)
      const directDebtAmount = 200n * USDC_DEC;
      await liquidityPool.connect(directBorrower).borrowDirect(usdc, directDebtAmount);
      await usdc.connect(directBorrower).transferFrom(liquidityPool, directBorrower, directDebtAmount);
      // balance = 300, directDebt = 200, virtualBalance = 500 > 100
      // profit = Math.min(300, 500 - 100) = Math.min(300, 400) = 300

      const profitAmount = 300n * USDC_DEC;
      await expect(liquidityPool.connect(withdrawProfit).withdrawProfit([usdc], user))
        .to.emit(liquidityPool, "ProfitWithdrawn").withArgs(usdc.target, user.address, profitAmount);
      expect(await usdc.balanceOf(user)).to.eq(profitAmount);
    });

    it("Should allow DIRECT_BORROW_ROLE to borrow direct", async function () {
      const {
      ...
      expect(await liquidityPool.balance(usdc)).to.eq(amountLiquidity - amountToBorrow);
      expect(await liquidityPool.directDebt(usdc)).to.eq(amountToBorrow);
    });
    ...
    it("Should reduce direct debt if repaid", async function () {
      ...
      await expect(liquidityPool.connect(directBorrower).repayDirect([usdc], [amountToBorrow]))
        .to.emit(liquidityPool, "RepaidDirect").withArgs(usdc, amountToBorrow);

      expect(await liquidityPool.directDebt(usdc)).to.be.eq(0);
      expect(await usdc.balanceOf(directBorrower)).to.be.eq(0);
      expect(await usdc.balanceOf(liquidityPool)).to.be.eq(amountLiquidity);
    });
    ...
    it("Should partially reduce direct debt if repaid less than debt", async function () {
      ...
      expect(await liquidityPool.directDebt(usdc)).to.be.eq(remainingDebt);
      expect(await usdc.balanceOf(directBorrower)).to.be.eq(remainingDebt);
      expect(await usdc.balanceOf(liquidityPool)).to.be.eq(amountLiquidity - remainingDebt);
    });
    ...
    it("Should fully reduce direct debt if repaid more than debt", async function () {
      ...
      expect(await liquidityPool.directDebt(usdc)).to.be.eq(0);
      expect(await usdc.balanceOf(directBorrower)).to.be.eq(extraRepayment);
      expect(await usdc.balanceOf(liquidityPool)).to.be.eq(amountLiquidity);
    });
  ...
  describe("withdrawProfit - non-ASSET token", function () {
    it("1: no balance → NoProfit revert", async function () {
    ...
    it("2: donated balance, accruedProfit=0 → full balance as surplus, second call reverts", async function () {
      const {
        liquidityPool, eurc, EURC_DEC, eurcOwner, withdrawProfit, user,
      } = await loadFixture(deployAll);

      const donated = 7n * EURC_DEC;
      await eurc.connect(eurcOwner).transfer(liquidityPool, donated);
      // non-ASSET: withdrawableSurplus = currentBalance always; accruedProfit=0 → return surplus
    ...
    it("3: multiple non-ASSET tokens donated → each fully withdrawn, second call reverts", async function () {
      const {
        liquidityPool, eurc, gho, EURC_DEC, GHO_DEC, eurcOwner, ghoOwner, withdrawProfit, user,
      } = await loadFixture(deployAll);

      const donatedEurc = 3n * EURC_DEC;
      const donatedGho = 5n * GHO_DEC;
      await eurc.connect(eurcOwner).transfer(liquidityPool, donatedEurc);
      await gho.connect(ghoOwner).transfer(liquidityPool, donatedGho);

      expect(await liquidityPool.accruedProfit(eurc)).to.eq(0n);
      expect(await liquidityPool.accruedProfit(gho)).to.eq(0n);
      const tx = await liquidityPool.connect(withdrawProfit).withdrawProfit([eurc, gho], user);
      await expect(tx).to.emit(liquidityPool, "ProfitWithdrawn").withArgs(eurc.target, user.address, donatedEurc);
      await expect(tx).to.emit(liquidityPool, "ProfitWithdrawn").withArgs(gho.target, user.address, donatedGho);
  ...
  describe("withdrawProfit - ASSET token accrued profit", function () {
    // Signs and executes a signed borrow; mockTarget receives the approved tokens.
    ...
    it("1: accruedProfit=0, balance equals deposited → NoProfit revert", async function () {
      const {
      ...
      // accruedProfit=0, balance=1000=deposited, no surplus
    ...
    it("2: accruedProfit=0, donated surplus → withdraws surplus, second call reverts", async function () {
      const {
        liquidityPool, usdc, USDC_DEC, usdcOwner, liquidityAdmin, withdrawProfit, user,
      } = await loadFixture(deployAll);
      ...
      // accruedProfit=0, balance=1050, virtualBalance=1050>1000, surplus=50
    ...
    it("3: accruedProfit>0, full balance available → withdraws exact profit, second call reverts",
      async function () {
    ...
    it("4: accruedProfit>0, surplus > profit → surplus amount withdrawn, accruedProfit cleared",
      async function () {
        const fixture = await loadFixture(deployAll);
        const {liquidityPool, usdc, USDC_DEC, usdcOwner, liquidityAdmin, withdrawProfit, user} = fixture;
        ...
        // accruedProfit=5, balance=1020, surplus=20, toWithdraw=5, max(5,20)=20
    ...
    it("5: accruedProfit>0, profit > surplus → profit amount withdrawn, accruedProfit cleared",
      async function () {
        const fixture = await loadFixture(deployAll);
        const {liquidityPool, usdc, USDC_DEC, usdcOwner, liquidityAdmin, withdrawProfit, user} = fixture;
    ...
    it("6: accruedProfit>0, balance < profit → partial withdrawal, immediate second call reverts",
      async function () {
        const fixture = await loadFixture(deployAll);
        const {liquidityPool, usdc, USDC_DEC, usdcOwner, liquidityAdmin, withdrawProfit, user} = fixture;
        ...
        // toWithdraw=min(7,10)=7, accruedProfit→3
    ...
    it("7: accruedProfit>0, balance=0 (all borrowed, nothing repaid) → NoProfit revert",
      async function () {
        const fixture = await loadFixture(deployAll);
        const {liquidityPool, usdc, USDC_DEC, usdcOwner, liquidityAdmin, withdrawProfit, user} = fixture;
    ...
    it("8: borrowDirect with profit → directDebt creates virtual surplus, profit withdrawn before repayment",
      async function () {
        const fixture = await loadFixture(deployAll);
        const {
        ...
        // balance=900, directDebt=105 (amount+profit), accruedProfit=5
        // virtualBalance=900+105=1005>1000, surplus=min(5,900)=5
        // toWithdraw=min(900,5)=5, max(5,5)=5
    ...
    it("9: borrowDirect with no profit → directDebt equals principal only, no surplus → NoProfit revert",
      async function () {
        const {
          liquidityPool, usdc, USDC_DEC, usdcOwner, liquidityAdmin, withdrawProfit, user, directBorrower,
        } = await loadFixture(deployAll);
    ...
    it("10: multiple borrows accumulate accruedProfit → full aggregate withdrawn, second call reverts",
      async function () {
        const fixture = await loadFixture(deployAll);
        const {liquidityPool, usdc, USDC_DEC, usdcOwner, liquidityAdmin, withdrawProfit, user} = fixture;
        ...
        // accruedProfit=8, balance=1008, surplus=min(8,1008)=8, toWithdraw=8, max(8,8)=8

        const totalProfit = profit1 + profit2;
        expect(await liquidityPool.accruedProfit(usdc)).to.eq(totalProfit);
        await expect(liquidityPool.connect(withdrawProfit).withdrawProfit([usdc], user))
