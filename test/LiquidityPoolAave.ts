...
describe("LiquidityPoolAave", function () {
  ...
  describe("Borrow, supply, repay, withdraw", function () {
    ...
    it("Should borrow a token", async function () {
      ...
      expectAlmostEqual(await liquidityPool.balance(usdc.target), 50n * USDC_DEC);
      expect(await liquidityPool.balance(eurc.target)).to.be.lessThan(availableBefore - amountToBorrow / 2n);
      expect(await liquidityPool.balance(eurc.target)).to.be.greaterThan(availableBefore - amountToBorrow * 2n);
    });
