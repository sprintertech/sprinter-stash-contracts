...
dotenv.config();
import {
  getHardhatNetworkConfig, getNetworkConfig, logDeployers, getDestinationRepayerAddresses
...
const main: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  let network: Network;
  console.log("Updating Repayer this-addresses");
  ({network} = await getNetworkConfig(validateDeployers));
  if (!network) {
  ...
  const targetAddress = (await hre.deployments.get(repayerId)).address;
  const target = await hre.ethers.getContractAt("Repayer", targetAddress);
...
main.tags = ["UpdateThisAddressesRepayer"];
export default main;
