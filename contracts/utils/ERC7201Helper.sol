...
library ERC7201Helper {
    ...
    function validateStorageLocation(bytes32 actual, string memory namespace) pure internal {
        bytes32 expected = getStorageLocation(bytes(namespace));
        require(actual == expected, InvalidStorageSlot(namespace));
    }

    function getStorageLocation(bytes memory namespace) pure internal returns(bytes32) {
